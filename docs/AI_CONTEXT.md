...
| Tests | Vitest |
| Font | Geist Mono (loaded via FontFace API before any canvas render) |
...
Main thread
  ...
  └─ Analysis sequence animation (requestAnimationFrame)
  └─ Canvas renderer (fingerprint + export)

Web Worker
  └─ OfflineAudioContext.decodeAudioData (decoded buffer stays in Worker)
  └─ FFT loop (2048-point, Hann window, 50% overlap)
...
**Critical invariant:** The decoded float32 AudioBuffer never crosses the Worker boundary. Only the compressed file bytes go in (Transferable); only the bar-level feature struct comes out. Violating this causes OOM on files >5 minutes.
...
interface BarData {
  ...
  pitchClassProfile: number[]; // 12-bin chromagram for this bar
  onsetDensity: number;    // normalized 0–1
  rms: number;             // 0–1
...
|--------|-------|--------|-------|
| `fft.ts` | Float32Array (2048 samples) | magnitude spectrum | Hann window applied before FFT |
| `chromagram.ts` | magnitude spectrum | 12-bin profile | STFT bins; bass below C3 is coarser |
...
| `bpm.ts` | onset strength signal | BPM | autocorrelation; robust to swing/rubato |
| `beats.ts` | onset strength + BPM | timestamp[] | dynamic programming beat tracker |
| `key.ts` | time-averaged chromagram | { key, confidence } | Krumhansl-Schmuckler |
| `barAggregation.ts` | beats + per-frame data | BarData[] | returns [] when BPM=0 |
...
- **Onset strength autocorrelation for BPM** — not onset-interval; robust to swing and rubato
- **Canvas 2D only** — SVG hybrid dropped due to cross-browser SVG-to-canvas font rendering failures
...
- BPM detection assumes constant tempo; graceful degradation unknown for tempo-changing music
- Analysis time for 10-min files is uncharted — benchmark in Phase 1
